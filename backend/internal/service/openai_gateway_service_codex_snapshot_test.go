...
func TestBuildCodexUsageExtraUpdates_PrimaryFiveHourUsedPercentStaysUsedPercent(t *testing.T) {
	primaryUsed := 10.0
	primaryReset := 3600
	primaryWindow := 300
	secondaryUsed := 34.0
	...
	snapshot := &OpenAICodexUsageSnapshot{
		...
		SecondaryResetAfterSeconds: &secondaryReset,
		SecondaryWindowMinutes:     &secondaryWindow,
		UpdatedAt:                  "2026-06-04T07:04:09Z",
	}
	...
	if got := updates["codex_primary_used_percent"]; got != 10.0 {
		t.Fatalf("codex_primary_used_percent = %v, want raw upstream value 10", got)
	}
	if got := updates["codex_5h_used_percent"]; got != 10.0 {
		t.Fatalf("codex_5h_used_percent = %v, want upstream used percent 10", got)
	}
...
func TestBuildCodexUsageExtraUpdates_SecondaryFiveHourUsedPercentStaysUsedPercent(t *testing.T) {
	primaryUsed := 93.0
	primaryReset := 86400
	primaryWindow := 10080
	secondaryUsed := 6.0
	...
	if got := updates["codex_5h_used_percent"]; got != 6.0 {
		t.Fatalf("codex_5h_used_percent = %v, want upstream used percent 6", got)
	}
